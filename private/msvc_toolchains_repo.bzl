...
# and MSVC Tools/lib/ directories of the active toolchain — recreating the
# linker search path that Bazel's auto-detected @local_config_cc Windows CC
# toolchain inherits from vcvars. The block is injected into per-frontend
# BUILD.toolchain.tpl as the {local_config_cc_compat_args} substitution when
# the local_config_cc_compat extension parameter is True.
# {target} placeholders are pre-substituted in Python (not ctx.template) to
# avoid clashing with cc_args.format keys.
def _local_config_cc_compat_args_block(target):
    return """
cc_args(
    name = "local_config_cc_compat_libpaths",
    actions = [
...
def _emit_winsdk_facades(ctx, winsdk_versions, hosts):
    ...
    inc_content = ["package(default_visibility = [\"//visibility:public\"])", ""]
    for inc in ["ucrt_include", "um_include", "shared_include", "cppwinrt_include", "winrt_include"]:
        inc_content.append("alias(\n    name = \"{}\",\n    actual = {},\n)\n".format(
            inc,
            _select_label("winsdk", winsdk_versions, lambda v, inc = inc: "@winsdk_{}//:{}".format(v, inc)),
        ))
    for inc in ["ucrt_include_files", "um_include_files", "shared_include_files", "cppwinrt_include_files", "winrt_include_files"]:
        inc_content.append("filegroup(\n    name = \"{}\",\n    srcs = {},\n)\n".format(
            inc,
...
""" + winsdk_cfg + _cc_imports(winsdk_libs)
    ...
    # filegroups (carry the .lib files into link inputs) for the local_config_cc_compat
...
""")
    ...
    data = [
        "@{llvm_repo}//:lld_link_exe_only_{suffix}",
                    ...
                    if ctx.attr.local_config_cc_compat:
    ...
    "/PDBALTPATH:%_PDB%",
                ...
                if ctx.attr.local_config_cc_compat:
                    local_config_cc_compat_args = _local_config_cc_compat_args_block(target)
                    local_config_cc_compat_toolchain_arg = '"local_config_cc_compat_libpaths",'
                else:
                ...
                ctx.template(
                    ...
                    substitutions = {
                        ...
                        "{base_link_flags}": base_link_flags,
                        "{local_config_cc_compat_args}": local_config_cc_compat_args,
                        "{local_config_cc_compat_toolchain_arg}": local_config_cc_compat_toolchain_arg,
                        "{target}": target,
                        "{host}": host,
                        "{suffix}": suffix,
...
msvc_toolchains_repo = repository_rule(
    ...
    attrs = {
        ...
        "local_config_cc_compat": attr.bool(default = False),
